...
- **Rounds — the far-right button** is the stage control: it names the next check owed (`✓ 1p chart`), and tapping it ticks that stage and rests the patient for 3 hours — the card dims and drops below everyone still owed. When the 3 h are up it comes back on its own with the next stage named. The dots on each row keep the running record of which stages are done. Tap the `◷ 2h 14m` timer to bring a patient back early, or `✓ 5/5` to reopen the last check.
